...
// Package main is the entrypoint for the Data Prepper operator.
package main
...
import (
	...
	"github.com/kaasops/dataprepper-operator/internal/controller"
	"github.com/kaasops/dataprepper-operator/internal/kafka"
	_ "github.com/kaasops/dataprepper-operator/internal/metrics" // register custom metrics
...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "1a4183ee.kaasops.io",
	...
	// Detect ServiceMonitor CRD availability
	...
	if err == nil {
		...
		setupLog.Info("ServiceMonitor CRD detected, enabling ServiceMonitor reconciliation")
	} else {
		setupLog.Info("ServiceMonitor CRD not found, ServiceMonitor reconciliation disabled")
	}
	...
	if err := (&controller.DataPrepperSourceDiscoveryReconciler{
		Client:             mgr.GetClient(),
		Scheme:             mgr.GetScheme(),
	...
	if err := (&controller.DataPrepperDefaultsReconciler{
		Client:   mgr.GetClient(),
		Scheme:   mgr.GetScheme(),
